...
1. **QUERY_DATABASE_TOOL schema conflict**
   - What we know: `"required": ["sql"]` but `list_tables=true` is a valid call without `sql`. The handler (agent.py:492) checks for this.
   - What's unclear: Whether Anthropic's tool-use API enforces `required` strictly or allows omitting required fields. If strict, Claude cannot call `list_tables` without also providing `sql`.
   - Recommendation: Remove `sql` from required, or restructure as two tools. Investigate during implementation.

2. **Total count query for cursor pagination**
   - What we know: The response envelope requires `"total": N`. Getting a total count requires a separate `SELECT COUNT(*)` query.
   - What's unclear: Whether the total count should be exact (extra query on every request) or omitted/cached.
   - Recommendation: Include exact count -- the datasets are small (documents, users, tasks are all small tables). If performance becomes an issue later, can cache or return -1.

3. **Frontend pagination integration**
   - What we know: Frontend `listDocuments()` currently expects `Document[]` not a paginated envelope.
   - What's unclear: Whether frontend changes are in scope for Phase 3.
   - Recommendation: Phase 3 changes the API contract. Frontend code in `web/src/api/client.ts` and `web/src/hooks/useDocuments.ts` will need minor updates to handle the new envelope. This is unavoidable -- the API is the source of truth. Keep frontend changes minimal (update types, handle `items` wrapper).
