...
- [x] **Phase 3: API + Agent Hardening** - Fix streaming, add response models and pagination, harden agent tools
- [x] **Phase 4: Frontend + Dead Code Cleanup** - Fix React rendering issues, accessibility, and remove dead code
- [x] **Phase 5: Audit Gap Closure** - Close partial requirements, remove dead frontend code, wire broken E2E flows
...
Plans:
- [x] 03-01-PLAN.md — Replace BaseHTTPMiddleware with pure ASGI, add SSE error events, fix _chunk_text
- [x] 03-02-PLAN.md — Cursor-based pagination, response_model additions, endpoint fixes (revoke_role 404, get_me 501, get_segment JOIN)
- [x] 03-03-PLAN.md — Agent tool schema fixes, CostTracker warning, cache key hash, SearchService Protocol, hybrid_search log fix
...
Plans:
- [x] 04-01-PLAN.md — React rendering fixes (stable keys, smart scroll, useCallback, setTimeout polling, Content-Type fix, chat aria-labels)
- [x] 04-02-PLAN.md — Accessibility for remaining components, dead code removal (CitationLink, require_auth), eval guard
...
Plans:
- [x] 05-01-PLAN.md — Backend type safety (deps.py cast, Protocol annotations) and conversation_id wiring
- [x] 05-02-PLAN.md — Frontend: dead code removal, ChatResponse alignment, metrics wiring, expandable details
...
| 3. API + Agent Hardening | 3/3 | ✓ Complete | 2026-02-18 |
| 4. Frontend + Dead Code Cleanup | 2/2 | ✓ Complete | 2026-02-18 |
| 5. Audit Gap Closure | 2/2 | ✓ Complete | 2026-02-18 |
