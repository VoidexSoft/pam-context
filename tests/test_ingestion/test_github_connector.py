...
class TestListDocuments:
    async def test_returns_md_and_txt_by_default(self, connector):
        with patch.object(connector, "run_cli", new_callable=AsyncMock, return_value=TREE_RESPONSE):
    ...
    async def test_filters_by_paths_and_extensions(self, connector_filtered):
        with patch.object(connector_filtered, "run_cli", new_callable=AsyncMock, return_value=TREE_RESPONSE):
    ...
    async def test_empty_tree(self, connector):
        with patch.object(connector, "run_cli", new_callable=AsyncMock, return_value={"tree": []}):
            docs = await connector.list_documents()
        assert docs == []
    ...
    async def test_calls_correct_api(self, connector):
        ...
        mock_run.assert_called_once_with(["api", "/repos/owner/repo/git/trees/main?recursive=1"])
...
class TestFetchDocument:
    async def test_returns_raw_document_for_markdown(self, connector):
        ...
        with patch.object(connector, "run_cli_raw", new_callable=AsyncMock, return_value=content):
    ...
    async def test_returns_plain_text_for_txt(self, connector):
        with patch.object(connector, "run_cli_raw", new_callable=AsyncMock, return_value=b"plain text"):
    ...
    async def test_calls_correct_api_with_raw_accept(self, connector):
        ...
        mock_raw.assert_called_once_with(
            [
                "api",
                "/repos/owner/repo/contents/docs/guide.md?ref=main",
                "-H",
    ...
    async def test_handles_nested_path(self, connector):
        with patch.object(connector, "run_cli_raw", new_callable=AsyncMock, return_value=b"deep"):
